...
export function AppHeader({ extras }: AppHeaderProps = {}) {
  ...
  const renderPageList = (item: AppNavPageList) => {
    ...
    return (
      <DropdownMenu
        ...
        label={navLabel(item)}
        open={open}
        onToggle={() => setOpenTrigger(o => (o === item.code ? null : item.code))}
        onClose={() => setOpenTrigger(o => (o === item.code ? null : o))}
        fullWidth={false}
      >
        <div className="dropdown-menu-list">
          {item.pages.map(p => {
            ...
            const target = buildViewUrl({ path: p.path, label }, liveSearch, viewParams);
            return (
              <Link
                key={p.path}
                to={target}
  ...
  const leftItems = navItems.filter(i => (i.align ?? 'left') === 'left');
  const rightItems = navItems.filter(i => i.align === 'right');
